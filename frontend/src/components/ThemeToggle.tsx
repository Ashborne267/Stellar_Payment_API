...
export default function ThemeToggle() {
  ...
  /**
   * Build a translated, human-readable description of the next theme.
   */
  ...
  const handleThemeToggle = useCallback(() => {
    ...
    toggleTheme();
  }, [toggleTheme, error, clearError, t, getNextThemeLabel]);

  /** Announce current theme to screen readers after each change. */
  useEffect(() => {
    if (isMounted && !isLoading && !error) {
      ...
      setAnnouncement(t("currentTheme", { theme: desc }));
    }
  ...
  const getTitle = () => {
    if (error) return t("errorTitle", { error: String(error) });
    if (theme === "system") {
      return t("titleSystem", { theme: t(`themeNames.${resolvedTheme ?? "light"}`) });
    }
    return theme === "light" ? t("titleLight") : t("titleDark");
  };
